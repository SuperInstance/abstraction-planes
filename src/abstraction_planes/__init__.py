...
Usage:
  python -m abstraction_planes "navigate east 10 knots, monitor reactor, alert if overheating"
  python -m abstraction_planes --target esp32 "sensor reader for temperature"
  python -m abstraction_planes --target cloud "fleet coordination protocol"
"""
...
SFKEY = os.environ.get("SILICONFLOW_KEY", "")
DSKEY = os.environ.get("DEEPSEEK_KEY", "")
...
# FLUX opcodes for bytecode decomposition
FLUX_OPS = {
    "MOVI": "0x10", "MOV": "0x11", "IADD": "0x20", "ISUB": "0x21",
    "IMUL": "0x22", "JMP": "0x30", "JZ": "0x31", "JNZ": "0x32",
    "CMP": "0x40", "GAUGE": "0x90", "ALERT": "0x91", "EVOLVE": "0xA0",
    "SAY": "0x80", "HALT": "0x01"
}
...
def call_qwen(prompt, temp=0.0, max_tok=300):
    """Call SiliconFlow Qwen API for chat completions."""
    body = json.dumps({
...
def main():
    ...
    if not intent_parts:
        print("Usage: python -m abstraction_planes [--target esp32|jetson|cloud|mud|agent|auto] \"intent description\"")
        sys.exit(1)
    ...
    # Save result
    outdir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "research")
    os.makedirs(outdir, exist_ok=True)
    outfile = os.path.join(outdir, f"plane-analysis-{datetime.utcnow().strftime('%Y%m%d-%H%M')}.json")
